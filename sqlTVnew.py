                ...
                if len(vidsrc_dict["result"]) != 0:
                    ...
                    for y in vidsrc_dict["result"]:
                        ...
                        if (id,) in imdb_8db:
                            same_imdb_id+=1
                            a+=1
                            print("已有",a,same_imdb_id,id)
                            break
                        else:
                            same_imdb_id=0
                            vod_name=y["show_title"]
                            play_url_1=y["embed_url"]
                            season =y["season"]
                            episode=y["episode"]
                            basic_info = [id,vod_name,play_url_1,season,episode]
                            sql = "insert into imdb_8 (id,vod_name,play_url_1,season,episode) values (%s,%s,%s,%s,%s)"
                            val= basic_info
                            mycursor.execute(sql, val)
                            mydb.commit()
                            a+=1
                            print("新的",a,same_imdb_id,id)
    #根据主表记录，清掉副表所有的不存在于主表的sid
        ...
        for (x,y,z,b) in imdb_8db:
            ...
            if (imdb_sid,) in imdb_macvod:
                basic_info = [b,x]
                sql = "update mac_vod set vod_play_url=%s where imdb_sid =%s"
                val= basic_info
                mycursor.execute(sql, val)
                mydb.commit()
                a+=1
        ...
        imdb_8db = mycursor.fetchall()
        a=0
        movie_genre_dict={'Western': 100, 'Comedy': 95, 'Action': 98, 'Adventure': 97, 'Documentary': 101, 'Family': 112, 'Crime': 99, 'Drama': 127, 'Animation': 106, 'Fantasy': 103, 'Sci-Fi': 111, 'Horror': 102, 'Biography': 104, 'Thriller': 115, 'N/A': 0, 'Music': 108, 'Mystery': 107, 'Reality-TV': 117, 'Game-Show': 118, 'Talk-Show': 119, 'News': 120, 'Romance': 126, 'Short': 96, 'History': 105, 'Sport': 121, 'Musical': 110, 'War': 114, 'N/A': 109, 'ungenre': 109}
        for (x,y) in imdb_8db:
