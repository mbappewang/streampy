...
try:
    ...
    for page in range(1,pages+1):
        try:
            vidsrc_dict = get_vidsrc("https://vidsrc.me/episodes/latest/page-" + str(page) + ".json")
            vidsrc_result = vidsrc_dict["result"]
        except:
            print(page,"超时")
        else:
            ...
            for info in vidsrc_result:
                if (info["embed_url"],) in imdb_8db:
                    print("已存在",info["imdb_id"],info["show_title"])
                    pass
                else:
                    basic_info=[info["imdb_id"],info["show_title"],info["embed_url"],info["season"],info["episode"]]
                    sql = "insert into tv (imdb_id,vod_name,play_url_1,season,episode) values (%s,%s,%s,%s,%s)"
                    val= basic_info
                    mycursor.execute(sql, val)
                    mydb.commit()
                    print("新加入",info["imdb_id"],info["show_title"],info["season"],info["episode"])